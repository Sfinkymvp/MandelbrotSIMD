...
def load_hwinfo_csv(path):
    # Добавлена cp1251 (Windows-1251) в начало списка
...
# Очистка заголовков: убираем кавычки и лишние пробелы
data.columns = [col.replace('"', '').strip() for col in data.columns]
# Убираем все не-ASCII символы из названий, кроме значка градуса
...
# --- ФИНАЛЬНАЯ ОЧИСТКА ТИПОВ ДАННЫХ ---

# 1. Превращаем значения в числа (ошибки станут NaN)
data[actual_target_col] = pd.to_numeric(data[actual_target_col], errors='coerce')

# 2. Выкидываем все строки, где нет данных или времени
data = data.dropna(subset=[actual_target_col])
if actual_time_col:
    data = data.dropna(subset=[actual_time_col])

# 3. Чтобы Matplotlib не путался, передаем данные как чистые списки Python
y_values = data[actual_target_col].tolist()

if actual_time_col:
    # Все метки времени принудительно делаем строками и убираем любые NaN
...
# Передача списков (list) вместо Series часто лечит TypeError в Matplotlib
